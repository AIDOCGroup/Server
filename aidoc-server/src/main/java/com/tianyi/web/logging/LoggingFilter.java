...
public class LoggingFilter implements Filter {
    ...
    @Value("${elk.log.enable}")
    private boolean logEnable;

    static {
        OBJECT_MAPPER.setSerializationInclusion(Include.NON_EMPTY);
    }
    public  Logger log = getLogger(LoggingFilter.class);
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws IOException, ServletException {
        ...
        logMap.put("VisitDate",new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:SSS").format(new Date()));
        logMap.put("Method",httpRequest.getMethod());
        logMap.put("RequestURI",httpRequest.getRequestURI());


        String req = getRequestDescription(requestWrapper);
        logMap.put("Request",Tools.parserJson2Map(req));
        logMap.put("error","");
        ...
        try {
            ...
            log.error("error", e);
            logMap.put("errorMsg",e.toString());

        } finally {
            String res =getResponseDescription(responseWrapper);
            logMap.put("Response",Tools.parserJson2Map(res));
            sb.append("\n").append(res);
        }
        ...
        try {
            //写入日志
            if (logEnable) {
                //HttpUtil.doPost("http://127.0.0.1:9200/tianyi/restLog", logMap);
            }
        } catch (Exception e) {
            log.error("error", e);
        }
