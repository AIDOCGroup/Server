...
public class AccountDetailDao extends BaseDao<AccountDetail> {
    ...
    public long getStepsDayMoney(long userId,String day) {
        ...
        BigDecimal num = execute(session -> (BigDecimal) session
            .createNativeQuery("select sum(ad.reward_amount)  FROM account_detail ad ,account a where ad.account_id = a.id and channel='GIVE' and a.user_id =:userId and DATE_FORMAT(ad.created_on,'%Y-%m-%d') =:day")
            .setParameter("day", day)
            .setParameter("userId",userId)
            .getSingleResult());
